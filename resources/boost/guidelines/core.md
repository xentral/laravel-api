...
- Opted-in endpoints also pass `groupSchemaName:` to `FilterParameter` (e.g. `groupSchemaName: 'ProductFilter'`), which emits `{name}Condition` plus an acyclic chain of group components — `{name}Group`, `{name}GroupDepth2` … `{name}GroupDepth5` — and points the parameter at `{name}Group`. The chain is unrolled to `MAX_GROUP_DEPTH` instead of a self-referencing `$ref` because spec tooling that dereferences into a plain object tree cannot serialise a cycle. Without the name the spec still declares the old single-level group.
